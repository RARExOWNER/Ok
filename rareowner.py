...
error_channel_id = -1002171264563
...
def create_inline_keyboard():
    ...
    # Create the buttons with their respective text (in capital letters) and URLs
    button3 = types.InlineKeyboardButton(
        text="🔥 𝗝𝗢𝗜𝗡 𝗢𝗨𝗥 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 🔥", 
        url="https://t.me/RARECRACKS"
    )
    button1 = types.InlineKeyboardButton(
        text="👤 𝗖𝗢𝗡𝗧𝗔𝗖𝗧 𝗢𝗪𝗡𝗘𝗥 👤", 
        url="https://t.me/RARExxOWNER"
    )
    button4 = types.InlineKeyboardButton(
        text="🔗 𝗝𝗢𝗜𝗡 𝗢𝗨𝗥 𝗚𝗥𝗢𝗨𝗣 🔗", 
        url="https://t.me/freerareddos"
    )
    ...
    markup.add(button3)  # Add first button (Channel)
    markup.add(button1)  # Add second button (Contact Owner)
    markup.add(button4)  # Add third button (Join Group)
    markup.add(button5)  # Add fourth button (Join Scammers Hell)

    # Return the markup (keyboard layout)
    return markup
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                       ...
                                       "*Contact the owner for assistance: @RARExxOWNER.*", 
                                       reply_markup=create_inline_keyboard(), parse_mode='Markdown')
            return
